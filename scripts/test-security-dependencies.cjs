...
async function main() {
  ...
  const evaluatorPackage = JSON.parse(
    fs.readFileSync(path.join(path.dirname(require.resolve('expr-eval')), '..', 'package.json'), 'utf8'),
  );
  ...
  assert.throws(() => Parser.evaluate('callback()', { callback: () => 1 }), /allowed function/i);
  assert.throws(() => Parser.evaluate('constructor'), /prototype access/i);

  const evaluatorUrl = pathToFileURL(path.join(__dirname, '..', 'custom-overrides', 'renderer', 'assets', 'safe-expression-evaluator.mjs'));
  ...
  for (const unsafeExpression of ['constructor', 'callback()', '1;globalThis.compromised=true', '1 / 0']) {
