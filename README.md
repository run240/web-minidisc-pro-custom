...
WinUSB installation flow on Windows. The current Windows source also includes
experimental mounted-storage support for the Sony NW-A3000 and NW-A1000, plus
a wide graphite-and-rose interface designed for desktop use.
...
a longer Hi-MD finalization window for the MZ-RH10 on Windows. It also contains
experimentally tested NW-A3000/NW-A1000 track transfer and deletion through the
player's mounted USB storage, together with the new wide rounded Windows UI.
These changes will not be present in the downloadable R8 ZIP until a newer
release is built.

> Network Walkman write support is experimental and currently limited to the
> tested NW-A3000 (`054c:0269`) and NW-A1000 (`054c:026a`) profiles. Keep a
> backup of the player before testing; other Network Walkman models remain
> unsupported unless explicitly listed.
...
- Sony NW-A3000·NW-A1000의 USB 저장소 기반 음악 전송·삭제 실험 지원
- 넓은 데스크톱 레이아웃, 라운드 카드와 전송 진행창을 적용한 다크 UI

> Network Walkman 쓰기 기능은 실험 기능입니다. 현재 실기 확인된 대상은
> NW-A3000(`054c:0269`)과 NW-A1000(`054c:026a`)이며, 시험 전 플레이어의
> 데이터를 별도로 백업하세요.
...
- Experimental mounted-storage transfer and deletion for Sony NW-A3000 and
  NW-A1000, restricted to explicitly tested USB profiles
- Wide rounded Windows workspace and transfer-progress dialogs using the
  graphite-and-rose custom theme
